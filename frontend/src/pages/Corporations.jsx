...
const Corporations = () => {
  ...
  const fetchCorporations = async () => {
    ...
    try {
      ...
      toast.error(err.response?.data?.message || "Failed to load corporations");
      setCorporations([]);
      setTotalPages(1);
      setTotalItems(0);
    } finally {
  ...
  const openAddForm = () => {
    setFormData({ corporation_name: '', status: 'Active' });
    setIsEditMode(false);
    setIsFormOpen(true);
  };

  const openEditForm = (corp) => {
    setFormData({
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!formData.corporation_name.trim()) {
      toast.error("Corporation name is required");
      return;
    }
    if (formData.corporation_name.length > 100) {
      toast.error("Corporation name cannot exceed 100 characters");
      return;
    }
    ...
    try {
      if (isEditMode) {
        await api.put(`/corporations/${selectedId}`, formData);
        toast.success("Corporation updated successfully");
      } else {
        await api.post('/corporations', formData);
  ...
  const toggleStatus = async (corp) => {
    const newStatus = corp.status === 'Active' ? 'Inactive' : 'Active';
    try {
      await api.patch(`/corporations/${corp.id}/status`, { status: newStatus });
  ...
  const deleteCorporation = async () => {
    try {
      await api.delete(`/corporations/${idToDelete}`);
      toast.success("Corporation deleted successfully");
      setIsDeleteModalOpen(false);
      fetchCorporations();
    } catch (err) {
      toast.error(err.response?.data?.message || "Failed to delete corporation");
    }
  };
  ...
  return (
    <div className="w-full">
      {isFormOpen ? (
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-300">
          <div className="p-6 border-b border-slate-100 flex justify-between items-center">
            <h2 className="text-xl font-bold text-slate-800">{isEditMode ? 'Edit Corporation' : 'Add New Corporation'}</h2>
          ...
          <form onSubmit={handleSubmit} className="p-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="space-y-6">
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Corporation Name *</label>
                  <input
                    type="text" name="corporation_name" value={formData.corporation_name} onChange={handleInputChange} required
            ...
            <div className="flex justify-end space-x-4 mt-10 pt-8 border-t border-slate-100">
              ...
              <button type="submit" disabled={submitting} className="flex items-center px-8 py-2.5 rounded-xl font-bold bg-[#7c3aed] text-white hover:bg-purple-700 shadow-lg shadow-purple-100 transition-all disabled:opacity-50">
                <Save className="w-4 h-4 mr-2" /> {submitting ? 'Saving...' : 'Save Corporation'}
              </button>
            </div>
          </form>
        </div>
        ...
        <>
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
            <div>
              <h1 className="text-2xl font-black text-slate-800 tracking-tight">Corporation Management</h1>
              <p className="text-slate-500 mt-1 text-sm">Add and manage administrative corporations.</p>
            </div>
            <button
              onClick={openAddForm}
            ...
            >
              <Plus className="w-5 h-5 mr-2" /> Add Corporation
            </button>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            ...
            <div className="p-4 border-b border-slate-100 flex flex-col md:flex-row gap-4 items-center">
              <form onSubmit={handleSearch} className="relative flex-1">
                ...
                <input
                  type="text" placeholder="Search corporations..." value={search} onChange={(e) => setSearch(e.target.value)}
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-50/50 border-b border-slate-100 text-[10px] uppercase font-black text-slate-400 tracking-widest">
                    ...
                    <th className="p-5 cursor-pointer select-none" onClick={() => handleSort('corporation_name')}>
                      Corporation Name {sortField === 'corporation_name' && (sortOrder === 'ASC' ? '▲' : '▼')}
                    </th>
                ...
                <tbody className="divide-y divide-slate-50">
                  {loading ? (
                    ...
                    <tr>
                      <td colSpan="5" className="p-20 text-center">
                        <p className="text-slate-600 font-bold">No Corporations Found</p>
                        <p className="text-sm text-slate-400 mt-1">Try adjusting your search or add a new corporation.</p>
                      </td>
                    </tr>
                  ) : corporations.map((corp, index) => (
                    <tr key={corp.id} className="hover:bg-slate-50/50 transition-colors group">
                      <td className="p-5 text-xs font-mono text-slate-400">{(page - 1) * 10 + index + 1}</td>
                      <td className="p-5">
                        <p className="font-bold text-slate-800 text-sm">{corp.corporation_name}</p>
                      </td>
                      <td className="p-5">
                        <button
                          onClick={() => toggleStatus(corp)}
                          className={`inline-flex items-center px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider border transition-all ${corp.status === 'Active'
                              ? 'bg-emerald-50 border-emerald-100 text-emerald-600 hover:bg-emerald-100'
                              : 'bg-slate-50 border-slate-100 text-slate-400 hover:bg-slate-100'
                            }`}
                        >
                          <span className={`w-1.5 h-1.5 rounded-full mr-2 ${corp.status === 'Active' ? 'bg-emerald-500' : 'bg-slate-300'}`}></span>
                          {corp.status}
                        </button>
                      </td>
                      <td className="p-5 text-xs text-slate-500 font-medium">
                        {new Date(corp.created_at).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })}
                      </td>
                      <td className="p-5 text-right space-x-2">
                        <button onClick={() => openEditForm(corp)} className="p-2 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-xl transition-all">
                          <Edit2 className="w-4 h-4" />
                        </button>
                        <button onClick={() => confirmDelete(corp.id)} className="p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-xl transition-all">
            ...
            {!loading && totalPages > 1 && (
              <div className="p-5 border-t border-slate-100 bg-slate-50/30 flex flex-col md:flex-row justify-between items-center gap-4">
                <p className="text-xs font-bold text-slate-400">
                  Showing {corporations.length} of {totalItems} items
                </p>
      ...
      <ConfirmModal
        ...
        message="This action will delete the corporation. This action cannot be undone if there are no linked records."
        onConfirm={deleteCorporation}
